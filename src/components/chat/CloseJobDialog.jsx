...
function CloseJobDialog({setMessageState, id ,jobId }) {
    ...
    const onSubmit = async (values) => {
        ...
        if (values.type == "close") {
            response = await axios.patch("/closeJob", values);
            setMessageState("closed");
            console.log("closeJob", response.data);
            
        } else {
            response = await axios.patch("/cancelJob", values);
            setMessageState("canceled");
                    ...
                    <Form {...form}>
                        ...
                        >
                            ...
                            <DialogHeader className="flex flex-col gap-2">
                                ...
                                {type == "close" ? (
                                    <DialogDescription>
                                        if the job is complete, you can close it
                                        to notify us. And please take a moment
                                        to rate and review the geoworker, this
                                        can help future clients.
                                    </DialogDescription>
                                ) : (
                                    <DialogDescription>
                                        if you want to close the job even if it
                                        wasn't done by the geoworker. Please
                                        take a moment to share the reason for
                                        that, this can help future clients.
                                    </DialogDescription>
                                )}
                            </DialogHeader>
