...
const MapCompo = () => {
  ...
  const handleChangeCenter = () => {
    ...
    function stepChangeCenter() {
      if (currentStep <= 1) {
        ...
        const newLat =
          mapCenter.lat + (targetCenter.lat - mapCenter.lat) * easedStep;
        const newLng =
          mapCenter.lng + (targetCenter.lng - mapCenter.lng) * easedStep;
  ...
  const resetToAlgiers = (e) => {
    ...
    function stepZoom() {
      if (currentStep <= 1) {
        const easedStep = easeInOutBack(currentStep);
        setZoom(
          zoom +
            (intermediaryZoom -
              zoom +
              (targetZoom - intermediaryZoom) * easedStep) *
              easedStep
        );
        setCenter(updateCenter(currentStep));
        currentStep += smoothZoomStep;
        setTimeout(stepZoom, smoothZoomTime);
      }
    }

    // Initial zoom out before zoom in
    function startZooming() {
      if (zoom > intermediaryZoom) {
        // Zoom out first to intermediary level
        let zoomOutStep = 0;
        function stepZoomOut() {
          if (zoomOutStep <= 1) {
            const easedStep = easeInOutBack(zoomOutStep);
            setZoom(zoom - (zoom - intermediaryZoom) * easedStep);
            setCenter(updateCenter(zoomOutStep));
            zoomOutStep += smoothZoomStep;
            setTimeout(stepZoomOut, smoothZoomTime);
          } else {
            // Once zoomed out, start zooming in
            currentStep = 0; // Reset step for zoom in
            stepZoom();
          }
        }
        stepZoomOut();
      } else {
        // If already below intermediary, just zoom in
        stepZoom();
      }
    }
  ...
  const handleZoomIn = () => {
    ...
    function stepZoomIn() {
      if (currentStep <= 1) {
        const easedStep = easeInOut(currentStep);
        setZoom(zoom + (targetZoom - zoom) * easedStep);
        currentStep += smoothZoomStep;
  ...
  const handleZoomOut = () => {
    ...
    function stepZoomOut() {
      if (currentStep <= 1) {
        const easedStep = easeInOut(currentStep);
        setZoom(zoom - (zoom - targetZoom) * easedStep);
        currentStep += smoothZoomStep;
        setTimeout(stepZoomOut, smoothZoomTime);
      }
    }
