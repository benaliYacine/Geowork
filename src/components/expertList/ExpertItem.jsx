...
export default function ExpertItem({ expert }) {
  ...
  const [isClient, setIsClient] = useState(expert.isClient ? true : false);
  const heartClick = async () => {
    let response;
    setClick(!isClick);
    if (!isClick) {
      response = await axios.patch(
        "/api/professionnels/addSavedProfessionnel",
        { id: expert.id }
      );
      console.log(response.data);
    } else {
      response = await axios.patch(
        "/api/professionnels/suppSavedProfessionnel",
        { id: expert.id }
      );
      console.log(response.data);
    }
      ...
      <div className="flex flex-col sm:flex-row items-center p-2 w-full">
        <div className="flex mr-auto">
          ...
          <div className="flex-grow mb-2">
            ...
            <div className="mb-1">
              <JobSuccess percentage={65} size="sm" />
            </div>
